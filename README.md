...
言語ごとに実行速度が異なるため、**基準値はバージョンごとに別々に補正済み**です。

| 実装 | シングルコア基準 | マルチコア基準 |
|---|---:|---:|
| `cpubench.sh`（シェル版） | 33,825.6 回/秒 | 128,829.6 回/秒 |
| `cpubench.py`（Python版） | 70,000 回/秒 | 320,000 回/秒 |
...
実機のCore i3-13100や、基準にしたい別のマシンで一度だけ実行します。
